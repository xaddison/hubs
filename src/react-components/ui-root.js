...
class UIRoot extends Component {
  ...
  performDirectEntryFlow = async enterInVR => {
    ...
    if (hasGrantedMic) {
      if (!this.audioManager.isAudioDeviceSelected) {
        await this.audioManager.setMediaStreamToDefault();
      }
  ...
  micDeviceChanged = async deviceId => {
    this.audioManager.selectMicDevice(deviceId);
  };
  ...
  onAudioReadyButton = async () => {
    ...
    await this.props.enterScene(this.audioManager.mediaStream, this.state.enterInVR, muteOnEntry);

    this.setState({ entered: true, entering: false, showShareDialog: false });

    const mediaStream = this.audioManager.mediaStream;

    if (mediaStream) {
      if (this.audioManager.audioTrack) {
        console.log(`Using microphone: ${this.audioManager.audioTrack.label}`);
      }
  ...
  renderAudioSetupPanel = () => {
    ...
    return (
      <MicSetupModalContainer
        ...
        selectedMicrophone={this.audioManager.selectedMicDeviceId}
        microphoneOptions={this.audioManager.micDevices}
        onChangeMicrophone={this.micDeviceChanged}
        microphoneEnabled={!!this.audioManager.audioTrack}
